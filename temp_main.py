...
async def upload_pdf(file: UploadFile = File(...)):
    try:
        ...
        doc_hash = doc_tracker.save_document(protocol_text, result.get("assignments", []))
...
<body>
    <div class="container">
        ...
        <p><strong>AI анализирует протокол и назначает курсы нужным userм</strong></p>
    ...
    <script>
        async function uploadPDF() {
            ...
            try {
                ...
                resultDiv.innerHTML = '<div class="result">❌ Error processing</div>';
            }
        }
                    ...
                    data.documents.forEach((doc, index) => {
                        ...
                        if (doc.assignments && doc.assignments.length > 0) {
                            ...
                            doc.assignments.forEach(assignment => {
                                ...
                                html += `🎯 Приоритет: ${assignment.priority} | ⏰ ${assignDate}`;
        ...
        async function showUsers() {
            ...
            try {
                ...
                resultDiv.innerHTML = '<div class="result">❌ Error loading пользователей</div>';
            }
        }
        ...
        async function showCourses() {
            ...
            try {
                ...
                resultDiv.innerHTML = '<div class="result">❌ Error loading курсов</div>';
            }
        }
        
        async function showAssignments() {
            const resultDiv = document.getElementById('result');
            resultDiv.innerHTML = '<div class="result">🎯 Загружаю assignment...</div>';
            
            try {
                const response = await fetch('/assignments-detail');
                const data = await response.json();
                
                let html = '<div class="result"><h3>🎯 Последние assignment (' + data.assignments.length + ')</h3>';
                ...
                resultDiv.innerHTML = '<div class="result">❌ Error loading назначений</div>';
            }
        }
        ...
        async function showCritical() {
            ...
            try {
                ...
                resultDiv.innerHTML = '<div class="result">❌ Error loading критичных курсов</div>';
            }
        }
        ...
        async function showActive() {
            ...
            try {
                ...
                resultDiv.innerHTML = '<div class="result">❌ Error loading активных курсов</div>';
            }
        }
        ...
        async function showCompletions() {
            ...
            try {
                ...
                resultDiv.innerHTML = '<div class="result">❌ Error loading завершений</div>';
            }
        }
        
        async function showUserProfile(userId) {
            const resultDiv = document.getElementById('result');
            resultDiv.innerHTML = '<div class="result">👤 Загружаю профиль user...</div>';
            ...
            try {
                ...
                console.error('Error:', error);
                resultDiv.innerHTML = '<div class="result">❌ Error loading профиля</div>';
            }
        }
        ...
        async function checkExpired() {
            ...
            try {
                ...
                if (data.expired_users && data.expired_users.length > 0) {
                    html += `<h4>⚠️ Найдено ${data.expired_count} пользователей с истекающими courseми:</h4>`;
                ...
                console.error('Error:', error);
                resultDiv.innerHTML = '<div class="result">❌ Error проверки сроков</div>';
            }
        }
        ...
        async function showAuditLog() {
            ...
            try {
                ...
                console.error('Error:', error);
                resultDiv.innerHTML = '<div class="result">❌ Error loading audit log</div>';
            }
        }
    </script>
</body>
...
async def get_users_detail():
    """Получить детальную информацию о userх"""
...
async def get_courses_detail():
    """Получить детальную информацию о courseх"""
...
async def get_assignments_detail():
    """Получить детальную информацию о assignmentх"""
...
async def get_document_history():
    ...
    for doc_hash, doc_info in doc_tracker.processed_docs.items():
        # Получаем логи для этого document
        doc_logs = [log for log in audit_logger.logs if log.get("document_hash") == doc_hash]
        
        # Получаем assignment по времени processing document
        doc_time = doc_info["processed_at"][:16]  # До минут
        assignments = [log for log in audit_logger.logs if 
                      log.get("action") == "course_assigned" and 
                      log.get("timestamp", "")[:16] == doc_time]
        ...
        documents.append({
            ...
            "assignments_count": doc_info["assignments_count"],
            "assignments": assignments
        })
...
async def get_completions_detail():
    """Получить детальную информацию о завершенных courseх"""
