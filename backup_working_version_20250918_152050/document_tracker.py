...
class DocumentTracker:
    ...
    def get_document_hash(self, text: str) -> str:
        """Создает хэш документа для определения дубликатов"""
        # Берем первые 1000 символов для хэширования
        content = text[:1000].strip().lower()
        return hashlib.md5(content.encode()).hexdigest()
    
    def is_duplicate(self, text: str) -> tuple[bool, Optional[Dict]]:
        """Проверяет, обрабатывался ли документ ранее"""
    ...
    def save_document(self, text: str, assignments: List[Dict], skipped_duplicates: List[Dict] = None) -> str:
        """Сохраняет информацию о обработанном документе"""
        doc_hash = self.get_document_hash(text)
        ...
        # Сохраняем историю назначений для каждого пользователя
    ...
    def get_user_history(self, user_id: str) -> List[Dict]:
        """Получает историю назначений для пользователя"""
